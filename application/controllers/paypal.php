...
class Paypal {
	...
	public function __construct($apiuser, $apipwd, $apisign, $returnurl = '', $cancelurl = ''){
	...
	public function SetExpressCheckout($params){
		...
		foreach($params['items'] as $items){
			$request .= '&L_NAME'.$c.'='.urlencode($items['itemname']);
			$request .= '&L_AMT'.$c.'='.urlencode($items['itemamt']);
			$request .= '&L_QTY'.$c.'='.urlencode($items['itemqty']);	
			$amt += $items['itemamt'] * $items['itemqty'];
			$c++;			
		}
	...
	{
		return 'www'.($this->sanbox ? '.sandbox' : '').'.paypal.com';
	}	
	
	private function apiurl()
	{
		return 'api-3t'.($this->sanbox ? '.sandbox' : '').'.paypal.com';
	}
